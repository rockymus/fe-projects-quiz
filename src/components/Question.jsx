...
export default function Question({ question, selectAnswer, gameOver }) {    
    ...
    const answerElements = question.answers.map(ans => <button disabled={gameOver && !(question.correctAnswer === ans && gameOver)} onClick={() => selectAnswer({...question, answer: ans})} className={clsx({
